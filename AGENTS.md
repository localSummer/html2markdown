...
- **侧栏**：按 tab 启用。`browser.sidePanel.open` 必须在用户手势的同步栈里调用，前面不能 `await`。工具栏图标或 `_execute_action`（默认 Alt+Shift+M）再点一次会关掉该 tab 的侧栏。快捷键改不了（Chrome 限制），设置页只展示并用 `tabs.create` 打开浏览器快捷键页。关掉该 tab 的侧栏时清当前页高亮；高亮开关不写入 settings，重开默认开。切到其他浏览器 tab 时只清旧页高亮，不要自动画到新页；该次会话里后续自动扫描也不再铺高亮。
