...
const alertVariants = cva(
  'relative w-full rounded-lg border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current',
  {
    variants: {
      variant: {
        default: 'bg-primary-soft/60 text-primary-soft-foreground border-primary/15',
        destructive:
          'bg-destructive-soft text-destructive border-destructive/20 [&>svg]:text-destructive',
        info: 'bg-primary-soft/50 text-primary-soft-foreground border-primary/20',
        warning:
