...
import { FRESH_STATE, type TabState } from './convert-types.ts';
...
export function ConvertTab({
  ...
  const getRefs = useCallback((id: number): TabRefs => {
    ...
    if (!r) {
      r = { markdown: '', abort: null, scanGen: 0, paintTimer: null };
  ...
  const hydrateFromHistory = useCallback(
    async (id: number, url: string | undefined) => {
      ...
      setStates((prev) => {
        ...
        if (cur.phase === 'scanning' || cur.phase === 'converting') return prev;
        if (cur.regions.length > 0 || (cur.markdown && !cur.fromHistory)) return prev;
        const r = getRefs(id);
  ...
  useEffect(() => {
    ...
    if (highlightOn && activeTabId !== undefined && active && active.regions.length > 0) {
  ...
  const canConvert = Boolean(settings.text.apiKey) && phase !== 'scanning' && !active.unsupported;
  ...
  const toggleHighlight = (on: boolean) => {
    ...
    } else if (on && activeTabId !== undefined && active && active.regions.length > 0) {
      void sendToTab(activeTabId, { type: 'HIGHLIGHT', region: active.selected }).catch(() => {});
    }
  };
  ...
  const scan = async () => {
    ...
    patchState(id, { phase: 'scanning', status: '扫描中…', error: '', progress: 0 });
    try {
      ...
      if (gen !== refs.scanGen) return;
      patchState(id, { phase: 'idle', error: err instanceof Error ? err.message : String(err) });
    }
  };
  ...
  const cancelScan = () => {
    refs.scanGen += 1;
    patchState(id, {
      phase: active.regions.length ? 'ready' : 'idle',
      status: '',
      error: '',
    });
  };
